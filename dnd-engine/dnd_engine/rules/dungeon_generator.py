...
class DungeonGenerator:
    ...
    def _generate_room_graph(
        self, num_rooms: int, theme: dict[str, Any]
    ...
    def _populate_rooms(
        self, rooms: dict[str, dict[str, Any]], theme: dict[str, Any], level: int
    ) -> None:
        """
        Populate rooms with enemies and items.
        ...
        for room_id, room in rooms.items():
            if room_id == "room_0":
                ...
                if roll < 0.3:
                    ...
                    room["enemies"] = [
                        random.choice(available_monsters) for _ in range(num_enemies)
                    ]
    ...
    def _find_safe_room(self, rooms: dict[str, dict[str, Any]], boss_room_id: str) -> str:
        ...
        candidates = [
            room_id for room_id in rooms.keys() if room_id not in ["room_0", boss_room_id]
        ]
    ...
    def _generate_loot(
        self, level: int, is_boss: bool = False, is_treasure: bool = False, is_minor: bool = False
    ) -> list[dict[str, Any]]:
        """
        ...
        if is_boss:
            ...
            if random.random() < 0.5:
                item_id = random.choice(
                    ["shortsword", "longsword", "dagger", "chain_shirt", "leather_armor"]
                )
